...
async function writeOutputs(rawImage, outputDir, baseName) {
  const masterPath = path.join(outputDir, `${baseName}-master.jpg`);
  const finalPath = path.join(outputDir, `${baseName}.jpg`);
  await sharp(rawImage.bytes).resize(1600, 800, { fit: 'cover', position: 'centre' }).jpeg({ quality: 92, mozjpeg: true }).toFile(masterPath);
  await sharp(masterPath).resize(400, 200, { fit: 'cover', position: 'centre' }).jpeg({ quality: 90, mozjpeg: true }).toFile(finalPath);
  return { masterPath, finalPath };
}

async function processProperty(property, runDir) {
  const name = cleanName(property.property_name);
  ...
  if (!chosen) {
    ...
    inspection = { score: null, usable: Boolean(chosen), reason: 'No usable downloadable reference image was found.' };
  }

  if (!chosen) throw new Error('No image could be produced.');
  const outputs = await writeOutputs(chosen, itemDir, name);
  return {
    ...
    inspection,
    image_url: `/runs/${path.basename(runDir)}/${name}/${path.basename(outputs.finalPath)}`,
    master_image_url: `/runs/${path.basename(runDir)}/${name}/${path.basename(outputs.masterPath)}`
  };
}
...
async function createZip(runDir, results) {
  ...
  await new Promise((resolve, reject) => {
    ...
    for (const result of results) {
      const folder = cleanName(result.property_name);
      const dir = path.join(runDir, folder);
      archive.file(path.join(dir, `${folder}.jpg`), { name: `${folder}/${folder}.jpg` });
      archive.file(path.join(dir, `${folder}-master.jpg`), { name: `${folder}/${folder}-master.jpg` });
    }
