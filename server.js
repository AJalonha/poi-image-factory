...
async function runJob(runId, properties, runDir) {
  const job = jobs.get(runId);
  try {
    job.status = 'processing';
    for (const property of properties) {
      try {
        job.results.push(await processProperty(property, runDir));
      } catch (error) {
        job.results.push({ ...property, error: error.message });
      }
      job.completed += 1;
    }
    const successful = job.results.filter((result) => result.image_url);
    job.zipUrl = successful.length ? await createZip(runDir, successful) : null;
...
app.post('/api/process', upload.single('csv'), async (req, res) => {
  try {
    ...
    // Start processing after returning the short acknowledgement response.
    void runJob(runId, properties, runDir);
    res.status(202).json({ runId, statusUrl: `/api/status/${runId}` });
  } catch (error) {
    console.error(error);
...
app.get('/api/status/:runId', (req, res) => {
  ...
  if (!job) return res.status(404).json({ error: 'Job not found. It may have expired after a server restart.' });
  res.json(job);
});
